        ...
        <form method="POST" action="">
            <div class="row g-3">
                ...
                <div class="form-label col-sm-4">
                  ...
                  <input type="number" class="form-control" name="cpf" id="cpf" id="floatingInput" placeholder="000.000.000-00" value="<?php echo $dados->cpf; ?>">
                  <div class="invalid-feedback">Digite um CPF válido</div>
                  <div class="col-12">
                    </br><input type="submit" class="btn btn-primary"  value="Pesquisar"/>
                </div>
                </div>

                <div class="col-sm-6">
                  <label for="cartaosuspac" class="form-label">Cartão SUS*</label>
                  <input type="number" name="cartaosuspac" class="form-control" id="cartaosuspac" placeholder="000 0000 0000 0000" value="<?php echo $dados->cpf; ?>" required>
               ...
               <div class="col-12">
                ...
                <?php
                        if(isset($_POST['cpf'])){
                          ...
                          WHERE cpf LIKE '%$_POST[cpf]%'";

                          $exec = mysqli_query($conn,$sql);
                          if($exec){
                              while($dados = mysqli_fetch_assoc($exec)){
                                  $cpf = $dados['cpf'];
                                  $cartao = $dados['cartaosuspac'];
                                  $nome = $dados['nomepac'];
                                  $email = $dados['emailpac'];
                                  $datanascimento = $dados['dtnascpac'];
                                  $telefone = $dados['telefonepac'];
                                  $celular = $dados['celularpac'];
                                  $genero = $dados['generopac'];
                                  $cor = $dados['cor'];
                                  $cep = $dados['ceppac'];
                                  $cidade = $dados['cidadepac'];
                                  $bairro = $dados['bairropac'];
                                  $uf = $dados['ufpac'];
                                  $endereco = $dados['enderecopac'];
                                  $complemento = $dados['complementopac'];
                                  $dependentes = $dados['possuideppac'];
                                  $quantdep = $dados['quntdep'];
                                  $diasemana = $dados['diadasemana'];
                                  $periodo = $dados['periodo'];
                                  $senha = $dados['senhapac1'];
